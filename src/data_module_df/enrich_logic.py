...
def select_new_samples(df_dataset: pd.DataFrame,
                       df_all: pd.DataFrame,
                       n_new_samples: int,
                       seed: int) -> pd.DataFrame:
    """
    Identifie et échantillonne de nouveaux échantillons depuis df_all 
    qui ne sont pas dans df_dataset, basé sur 'productid'.
    """
    np.random.seed(seed)

    # Identifier les nouveaux samples
    used_ids = set(df_dataset['productid'])
...
def enrich_dataset(df_old: pd.DataFrame,
